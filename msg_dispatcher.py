...
def log2file(jbody):
    ...
    log_file.write('[%s] Build job SRPM %s (%s) for arch %s builder %s status %s [%s] (Scratch job: %s) \r\n' % (time.asctime(), jbody[
                   'srpm'], jbody['timestamp'], jbody['arch'], jbody['builder_fqdn'], jbody['status'], jbody['evr'], jbody['scratch']))
    log_file.close()
...
def sendmail(jbody):
    ...
    try:
        root_log = urllib2.urlopen(
            "%s/%s/%s/%s/%s.%s/root.log" %
            (base_url, jbody['target'],
             jbody['pkgname'],
             jbody['timestamp'],
             jbody['evr'],
             jbody['arch']))
    ...
    try:
        build_log = urllib2.urlopen(
            "%s/%s/%s/%s/%s.%s/build.log" %
            (base_url, jbody['target'],
             jbody['pkgname'],
             jbody['timestamp'],
             jbody['evr'],
    ...
    msg['From'] = email_from
    msg['To'] = email_to
    msg['Subject'] = '[reimzul] Build task %s %s (arch: %s) target %s : %s' % (
        jbody['timestamp'], jbody['srpm'], jbody['arch'], jbody['target'], jbody['status'])
