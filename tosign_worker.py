...
def main():
  ...
  while True:
    try:
      ...
      job.delete()
      src_dir = reimzul_repo_basedir + jbody['target'] + '/' + jbody['pkgname'] + '/' + jbody['timestamp'] + '/'
      target_dir = reimzul_tosign_basedir + jbody['target'] + '/'
      print "Copying RPM pkgs from %s to %s" % (src_dir, target_dir)
      copy_cmd = "test -d %s || mkdir -p %s ; find %s -iname '*.rpm' -exec cp {} %s \;" % (target_dir, target_dir, src_dir , target_dir)
