...
def main():
    ...
    while True:
        try:
            ...
            os.close(rpm_file)
            jbody['evr'] = hdr[rpm.RPMTAG_VERSION] + \
            ...
            # launching job
            build_cmd = "/srv/reimzul/code/submit_mock.sh -s %s -d %s -t %s -a %s -p %s" % (
                local_srpm, jbody['disttag'], jbody['target'], jbody['arch'], timestamp)
            print build_cmd
            ...
            # Specific case : chaining i386/ppc automatically for x86_64/ppc64
            # builds
            if jbody['arch'] == 'x86_64' or jbody['arch'] == 'ppc64':
               ...
               # Specific kernel case (needs noarch builds after x86_64,i386
               # ones)
