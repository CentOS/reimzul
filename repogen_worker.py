...
def main():
    ...
    while True:
        try:
            ...
            createrepo_cmd = "test -d %s || mkdir -p %s ; test -f %s/.repolock && { echo other createrepo in progress; } || { touch %s/.repolock; time /usr/bin/createrepo_c -d --update --workers 64 --retain-old-md 3 --cachedir %s %s ; rm %s/.repolock; }" % (
                repodir, repodir, repodir, repodir, reimzul_repo_cachedir, repodir, repodir)
