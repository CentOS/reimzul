...
def build_srpm(jbody,topic,arch):
  ...
  os.chdir(tmp_dir)
  srpm_build_cmd = "/srv/reimzul/code/git-to-srpm.sh -p %s -b %s -c %s -u %s -d '%s' -s '%s'" % (jbody['pkg'],jbody['git_branch'],jbody['git_ref'],git_url,jbody['disttag'],jbody['sources_dir'])
